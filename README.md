...
The `InertiaRenderer` supports nonce-based CSP policies. When Mako's `ContentSecurityPolicy` middleware assigns a nonce to the `_csp_nonce_` view variable, the adapter applies it to the `<script>` and `<link>` tags generated by php-vite, so your Vite assets continue to load under a strict policy like the following:
...
Strict nonce-based CSP is only practical for production builds. When running Vite in development mode, Vite injects additional scripts at runtime (the HMR client and dynamically imported modules), and these won't carry your nonce. It's recommended to relax or disable the CSP in development.
